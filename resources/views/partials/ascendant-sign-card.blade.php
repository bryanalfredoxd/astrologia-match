...
<div class="bg-gradient-to-r from-[#4A0E7B] to-[#1A1F4D] backdrop-blur-sm rounded-2xl p-6 mb-6 shadow-lg">
    ...
    <div class="flex items-center justify-center mb-4">
        <h2 class="text-xl sm:text-2xl font-bold flex items-center text-center">
            <i class="fas fa-arrow-up text-[#FFD700] mr-3"></i> {{-- Icono de flecha hacia arriba para ascendente --}}
            Mi Signo Ascendente
        </h2>
    </div>

    @if(isset($ascendantSign) && $ascendantSign->nombre_signo !== 'Nada')
        ...
        <div class="grid grid-cols-3 gap-x-4 sm:gap-x-6 md:gap-x-16 justify-items-center mt-6 md:mt-8">
            {{-- Columna 1: Signo Ascendente (Imagen y Nombre) --}}
            <div class="flex flex-col items-center text-center">
                <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-[#0A0E2A] border-2 border-[#FFD700] flex items-center justify-center mb-2">
                    <?php
                        $signoSlug = isset($ascendantSign->nombre_signo) ? strtolower($ascendantSign->nombre_signo) : '';
                    ?>
                    <img src="{{ asset('images/zodiaco/' . $signoSlug . '.png') }}" alt="{{ $ascendantSign->nombre_signo ?? 'Signo Ascendente' }}" class="w-full h-full object-contain p-1">
                </div>
                <h4 class="text-base sm:text-lg font-bold text-[#FFD700]">
                    {{ $ascendantSign->nombre_signo ?? 'N/A' }}
                </h4>
            </div>
        ...
        <div class="mt-8 pt-4 border-t border-[#4A0E7B] text-center">
            <h3 class="text-lg font-bold text-[#FFD700] mb-3">Análisis de tu Signo Ascendente</h3>
            <div id="ascendant-analysis-text" class="text-[#A7B3EB] text-sm leading-relaxed">
...
<script>
    document.addEventListener('DOMContentLoaded', async function() {
        const ascendantSignData = {
            nombre_signo: "{{ $ascendantSign->nombre_signo ?? 'Nada' }}",
            elemento: "{{ $ascendantSign->elemento ?? '' }}",
            modalidad: "{{ $ascendantSign->modalidad ?? '' }}"
        };
        const analysisDiv = document.getElementById('ascendant-analysis-text');

        if (ascendantSignData.nombre_signo && ascendantSignData.nombre_signo !== 'Nada') {
            try {
                const prompt = `Genera un análisis astrológico conciso y revelador (máximo 150 palabras) para una persona con el Signo Ascendente ${ascendantSignData.nombre_signo}, Elemento ${ascendantSignData.elemento}, y Modalidad ${ascendantSignData.modalidad}. Enfócate en cómo estas características influyen en su personalidad externa, la primera impresión que da, su apariencia y cómo se proyecta al mundo. Ofrece una perspectiva positiva sobre cómo esta energía se manifiesta en su interacción social.`;
                ...
                if (result.candidates && result.candidates.length > 0 &&
                    ...
                    console.error('Error al generar análisis ascendente: Respuesta inesperada de la API', result);
                }
            } catch (error) {
                analysisDiv.innerHTML = '<span class="text-red-400">Error al cargar el análisis. Inténtalo de nuevo más tarde.</span>';
                console.error('Error al llamar a la API de Gemini para análisis ascendente:', error);
            }
        } else {
            analysisDiv.innerHTML = 'Análisis disponible una vez que se calcule tu Signo Ascendente.';
